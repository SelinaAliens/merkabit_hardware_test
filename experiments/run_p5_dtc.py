...
def run_dtc_sweep(backend, shots, q_fwd, q_inv, n_max, paired, epsilon, seed):
    """Run DTC measurement for n=1..n_max."""
    layout = [q_fwd, q_inv]
    zz_series = []
    raw_data = []
    ...
    for n in range(1, n_max + 1):
        ...
        label = f"DTC n={n} {mode}{eps_str}"

        if n % 6 == 0 or n <= 3 or n == n_max:
            print(f"\n-- {label} " + "-" * 30)
        ...
        raw_data.append({
            ...
            "counts": counts, "shots": total, "job_id": job.job_id(),
            "transpiled_depth": transpiled.depth(),
        })

        if n % 6 == 0 or n <= 3 or n == n_max:
            print(f"   <ZZ>={zz:+.4f}  P(00)={p00:.4f}  depth={transpiled.depth()}")
...
def main():
    ...
    zz_ctrl, raw_ctrl = run_dtc_sweep(
        backend, args.shots, args.q_fwd, args.q_inv,
        args.n_max, paired=False, epsilon=0.0, seed=args.seed)
